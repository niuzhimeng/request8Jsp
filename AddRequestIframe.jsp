...
<%@ page import="com.weaver.formmodel.util.Money" %>
...
<BODY id="flowbody"  <%if (ismode.equals("1") && modeid > 0) {%> onload="windowonload()" <%} else {%>
onload="addrequestiframeonload()"<%}%>>
<%if (!fromFlowDoc.equals("1")) {%>
...
<form name="frmmain" method="post" action="<%=operationpage%>">
    ...
    <div id="t_headother">
        <table class="ListStyle" cellspacing='1' border='0'>
            ...
            %>
            <tr class="datalight">
                <td><%=SystemEnv.getHtmlLabelName(16271, user.getLanguage())%><%=loanamount%>
                </td>
            </tr>
            </tbody>
        </table>
    ...
    <%}%>
    <input type="hidden" name="needwfback" id="needwfback" value="1"/>
    <input type="hidden" name="f_weaver_belongto_userid"
           value="<%=request.getParameter("f_weaver_belongto_userid") %>">
    <input type="hidden" name="f_weaver_belongto_usertype"
           value="<%=request.getParameter("f_weaver_belongto_usertype") %>">
    <input type="hidden" id="docFlags" value="<%=docFlagss%>"/>
</form>
<%
        }
...
%>
    ...
    function cptcusifover(_callback) {
        ...
        if (zcviewtype == 0) {//主字段
            ...
            var zcid = jQuery("input[name='field"+<%=zicFieldId %>+
            "']"
        ).
            val();//领用的资产id
            var cptnum = jQuery("input[name='field"+<%=shulFieldId %>+
            "']"
        ).
            val();//领用的数量
            ...
            if (zcviewtype == 2) {
                indexElement = "indexnum1";
            }
            else if (zcviewtype == 3) {
                indexElement = "indexnum2";
            }
            else if (zcviewtype == 4) {
                indexElement = "indexnum3";
            }
