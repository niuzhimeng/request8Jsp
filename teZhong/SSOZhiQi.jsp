...
<%
    ...
    baseBean.writeLog("智企单点oa Start============ " + TimeUtil.getCurrentTimeString());
    try {
        ...
        String[] params = {"common", "0", "biz.bizCheckSSO", projectDetails};
        ...
        String myStatus = rootElt.element("data").elementTextTrim("msg");

        if ("1".equals(myStatus)) {
            // 允许登录
            ...
            if (rs.next()) {
                ...
                if (user == null || !user.getLoginid().equals(pidAfter)) {
                    Frusername = rs.getString("lastname");
    ...
    baseBean.writeLog("智企单点oa End============  " + TimeUtil.getCurrentTimeString());
