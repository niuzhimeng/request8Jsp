...
// 查询请求的相关工作流基本信息
	...
	int isscan= Util.getIntValue(request.getParameter("isscan"),-1) ;
	String ccida = new WFLinkInfo().getCurrentNodeid(requestid,userid ,1)+"";
	
	if (isscan==1){
		String filepath ="qc451160";
		String formidd =WorkflowComInfo.getFormId(workflowid+"");
		int isbilld = Integer.parseInt(WorkflowComInfo.getIsBill(workflowid+""));
		WorkflowBillComInfo workflowBillComInfo =   new WorkflowBillComInfo();
		String cid = new BaseBean().getPropValue(filepath,"currentnodeid");
		List<String> nodelist = WorkflowVersion.getRelationNodeListByNodeID(ccida+"");
		if(cid.equals(ccida+"")||nodelist.contains(cid)) {

			String IsFinanceCollected = new BaseBean().getPropValue(filepath, "IsFinanceCollected");
			String tablename = "workflow_form";
			if (isbilld == 1)
				tablename = workflowBillComInfo.getTablename(formidd + "");


			try {

				String sql1 = "update " + tablename + " set " + IsFinanceCollected + " = '0' where requestid=" + requestid;
				rs.execute(sql1);
			} catch (Exception e) {
