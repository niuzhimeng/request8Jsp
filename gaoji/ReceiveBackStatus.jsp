...
<%
    ...
    try {
        ...
        if (poauth != null && oaauth.equals(poauth.trim())) {
            ...
            baseBean.writeLog("归档前接收sap返回状态接口执行 --- " + TimeUtil.getCurrentTimeString() + "接收的数据：" + json);
            ...
            insertSql.append(")");
            baseBean.writeLog("支付结果回写sap： " + insertSql.toString());
            RecordSet recordSet = new RecordSet();
            recordSet.execute(insertSql.toString());

            //更新流程表单状态字段

            JsonObject jsonObject2 = new JsonParser().parse(bdata).getAsJsonObject();
            if(jsonObject2.has("ZOAID")){
                String requestId = jsonObject2.get("ZOAID").isJsonNull() ? "" : jsonObject2.get("ZOAID").getAsString();
                String djbh = jsonObject2.get("ZDJBH").isJsonNull() ? "" : jsonObject2.get("ZDJBH").getAsString();
                String tableName = jsonObject2.get("ZTBNA").isJsonNull() ? "" : jsonObject2.get("ZTBNA").getAsString();
                String djStatus = jsonObject2.get("ZDJZT").isJsonNull() ? "" : jsonObject2.get("ZDJZT").getAsString();
                String epayStatus = jsonObject2.get("ZFKZT").isJsonNull() ? "" : jsonObject2.get("ZFKZT").getAsString();//1是审批打回，2是付款失败，3,退款打回，4是资金计划不足
                String czType = "submit";
                if (requestId != null && !"".equals(requestId) && tableName != null && !"".equals(tableName) && djStatus != null && !"".equals(djStatus)) {
                    recordSet.execute("select userid from workflow_currentoperator where requestid = '" + requestId + "'  and isremark = '0'");
                    int createId = 0;//归档前操作者id
                    if (recordSet.next()) {
                        createId = recordSet.getInt("userid");
                    }
                    if ("03".equals(djStatus)) {
                        if("1".equals(epayStatus)){
                            djStatus = "07";
                        } else if("2".equals(epayStatus)) {
                            djStatus = "03";
                        } else if("3".equals(epayStatus)) {
                            djStatus = "08";
                        } else if("4".equals(epayStatus)) {
                            djStatus = "05";
                        } else {
                            djStatus = "03";
                        }
                    ...
                    recordSet.execute("update " + tableName + " set zt = '" + djStatus + "' where requestId = '" + requestId + "'");

                    WorkflowServiceImpl workflowService = new WorkflowServiceImpl();
                    WorkflowRequestInfo workflowRequestInfo = new WorkflowRequestInfo();
                    if (!"02".equals(djStatus)) {
                        czType = "reject";
                    }
                    String result = workflowService.submitWorkflowRequest(workflowRequestInfo, Integer.parseInt(requestId.trim()), createId, czType, "ok");
                    baseBean.writeLog("归档前对流程操作状态： " + result + "-----------" + TimeUtil.getCurrentTimeString());
                    out.clear();
                    out.print(result);
                } else {
                    baseBean.writeLog("归档前对流程操作状态： 参数值为空-----------" + TimeUtil.getCurrentTimeString());
                    out.clear();
                    out.print("归档前对流程操作状态： 参数值为空");
                }
            } else {
                baseBean.writeLog("归档前对流程操作状态： ZOAID不存在-----------" + TimeUtil.getCurrentTimeString());
                out.clear();
                out.print("归档前对流程操作状态： ZOAID不存在");
            }
        } else {
            out.clear();
            out.print("访问凭证校验不通过");
        ...
        baseBean.writeLog("归档前接收sap返回状态异常： " + e);
        out.clear();
        out.print(e);
    }
