...
//add by mackjoe at 2006-04-24 td3994
    ...
    if (!canview && !isurger && !wfmonitor && !CoworkDAO.haveRightToViewWorkflow(Integer.toString(userid), Integer.toString(requestid))) {
        if (!WFUrgerManager.UrgerHaveWorkflowViewRight(desrequestid, userid, Util.getIntValue(logintype, 1)) && !WFUrgerManager.getMonitorViewRight(desrequestid, userid)) {//督办流程和监控流程的相关流程有查看权限
            ...
            RecordSet mySet = new RecordSet();
            mySet.executeQuery("select id from hrmresource where belongto = " + userid);
            while (mySet.next()) {
                accountBuilder.append(mySet.getInt("id")).append(",");
            }
            if (accountBuilder.length() > 0) {
                // 删除最后一位逗号
                accountBuilder.deleteCharAt(accountBuilder.length() - 1);
                String accountStr = accountBuilder.toString();
                // 查询参与流程的次账号
                mySet.executeQuery("select userid from workflow_currentoperator where requestid = " + requestid + " and userid in (" + accountStr + ")");
                if (mySet.next()) {
                    response.sendRedirect("/workflow/request/ViewRequest.jsp?requestid=" + requestid +
                            "&f_weaver_belongto_userid=" + mySet.getString("userid") + "&f_weaver_belongto_usertype=0&isovertime=0");
                    return;
                }
            }
...
<input type=hidden name=picInnerFrameurl
       value="/workflow/request/WorkflowRequestPictureInner.jsp?f_weaver_belongto_userid=<%=userid%>&f_weaver_belongto_usertype=<%=usertype%>&isview=1&fromFlowDoc=<%=fromFlowDoc%>&modeid=<%=modeid%>&requestid=<%=requestid%>&workflowid=<%=workflowid%>&nodeid=<%=nodeid%>&isbill=<%=isbill%>&formid=<%=formid%>&randpara=<%=System.currentTimeMillis()%>">
<input type=hidden name=statInnerFrameurl
       value="WorkflowRequestPicture.jsp?f_weaver_belongto_userid=<%=userid%>&f_weaver_belongto_usertype=<%=usertype%>&hasExt=true&requestid=<%=requestid%>&workflowid=<%=workflowid%>&nodeid=<%=nodeid%>&isbill=<%=isbill%>&formid=<%=formid%>&desrequestid=<%=desrequestid %>&isurger=<%=isurger%>&randpara=<%=System.currentTimeMillis()%>">
...
<TABLE width=100%>
    <tr>
        <td valign="top">
            <%if (message.equals("4")) {%>
            <font color=red><%=SystemEnv.getHtmlLabelName(21266, user.getLanguage())%>
            </font>
            <%} else if (message.equals("5")) {%>
            <font color=red><%=SystemEnv.getHtmlLabelName(21270, user.getLanguage())%>
            </font>
            <%} else if (message.equals("6")) {%>
            <font color=red><%=SystemEnv.getHtmlLabelName(21766, user.getLanguage())%>
            </font>
            <%} else if (message.equals("7")) {%>
            <font color=red><%=SystemEnv.getHtmlLabelName(22751, user.getLanguage())%>
            </font>
            <%} else if (message.equals("8")) {%>
            <font color=red><%=SystemEnv.getHtmlLabelName(24676, user.getLanguage())%>
            </font>
            <%}%>
