...
<%
	   ...
	   <script>
		jQuery(document).ready(function(){
			...
			jQuery(newLeaveType).autoSelect();
			}catch(e){}
		});
...
function initInfo(){
		...
		if(resourceId != ''){
			...
			ajax.send("operation=initInfo&bohai=true&resourceId="+resourceId+"&currentDate="+fromDate);
	...
	if("<%=showAnnualInfo%>" == "true" || "<%=showPSLInfo%>" == "true" || "<%=showPaidLeaveInfo%>" == "true") {
		...
		if($GetEle("<%=vacationLabel%>span")!=null){
			if("<%=showAnnualInfo%>" == "true") {
				...
				$GetEle("<%=vacationLabel%>span").innerHTML = "<%=SystemEnv.getHtmlLabelName(24039,user.getLanguage())+"&nbsp;:&nbsp;"+lastyearpsldays%><br><%=SystemEnv.getHtmlLabelName(24040,user.getLanguage())+"&nbsp;:&nbsp;"+thisyearpsldays%><br><%=SystemEnv.getHtmlLabelName(24041,user.getLanguage())+"&nbsp;:&nbsp;"+allpsldays%>";
			} else {
